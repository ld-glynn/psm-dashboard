  ...
  if (groupBy === "agent") {
    ...
    for (const [agentId, items] of Object.entries(byAgent)) {
      ...
      groups.push({
        ...
        sublabel: pattern ? `Solving: ${pattern.name}` : agent?.title || "",
        items,
      });
    }
    ...
    for (const o of outputs) {
      if (!byType[o.skill_type]) byType[o.skill_type] = [];
      byType[o.skill_type].push(o);
    }
    for (const [type, items] of Object.entries(byType)) {
      groups.push({
        key: type,
        label: skillTypeLabels[type] || type,
        sublabel: `${items.length} output${items.length !== 1 ? "s" : ""}`,
        items,
      });
    }
  } else {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-bold text-white">Skill Outputs</h1>
          <InfoTooltip text="Deliverables produced by Agent New Hires — action plans, recommendations, process docs, and investigations. These are the tangible work products of the pipeline." />
          <span className="text-xs text-white/30 ml-2">{outputs.length} deliverable{outputs.length !== 1 ? "s" : ""}</span>
        </div>
        <p className="text-sm text-white/40 mt-1">
          Review the actual outputs produced by each agent.
        </p>
      </div>
        ...
        {(["agent", "type", "flat"] as const).map((g) => (
          <button
            ...
            className={`px-2.5 py-1 text-xs rounded-md transition-colors ${
              groupBy === g ? "bg-white/10 text-white" : "text-white/40 hover:text-white/70 hover:bg-white/5"
            }`}
          >
      ...
      {outputs.length === 0 ? (
        <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl p-8 text-center">
          <p className="text-white/40">No skill outputs yet. Run the pipeline to generate deliverables.</p>
        </div>
      ) : (
