  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4">
      ...
      >
        ...
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Capability Inventory</h3>
        ...
        <span className="text-[10px] text-muted-foreground/50 ml-auto">v{inventory.version}</span>
      </button>
      ...
      {expanded && (
        <div className="mt-4 space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {inventory.agent_types.map((at) => (
                <div key={at.agent_type} className="bg-muted rounded-lg p-3">
                  <div className="text-xs font-medium text-foreground">{at.agent_type.replace(/_/g, " ")}</div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {at.skills.map((s) => (
                      <span key={s} className="text-[9px] px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300">{s.replace(/_/g, " ")}</span>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {inventory.skill_types.map((st) => (
                <div key={st.skill_type} className="bg-muted rounded-lg p-2.5">
                  <div className="text-xs font-medium text-foreground">{st.skill_type.replace(/_/g, " ")}</div>
          ...
          <div className="text-[10px] text-muted-foreground/50 pt-2 border-t border-border">
