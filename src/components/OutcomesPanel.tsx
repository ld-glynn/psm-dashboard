  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Outcome Tracking</h3>
      ...
      {!hasData ? (
        ...
        <div className="space-y-3">
          ...
          {Object.keys(summary.by_domain).length > 0 && (
            <div>
              <div className="text-[10px] text-muted-foreground/60 uppercase tracking-wide mb-1.5">By Domain</div>
              <div className="space-y-1">
                {Object.entries(summary.by_domain).map(([domain, stats]) => (
                  <div key={domain} className="flex items-center gap-2 text-[10px]">
                    <span className="text-muted-foreground w-20 capitalize">{domain}</span>
                    ...
                    <span className="text-muted-foreground w-12 text-right">{stats.validated}/{stats.total}</span>
                  </div>
                ))}
              </div>
            </div>
