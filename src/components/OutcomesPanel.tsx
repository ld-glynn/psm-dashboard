  ...
  return (
    <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider">Outcome Tracking</h3>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            <div className="bg-[#12121a] rounded-lg p-2.5 text-center">
              <div className="text-lg font-bold text-white/80">{summary.total + localTesting}</div>
              <div className="text-[10px] text-white/30">Tracked</div>
            </div>
            <div className="bg-[#12121a] rounded-lg p-2.5 text-center">
              <div className="text-lg font-bold text-blue-400">{localTesting}</div>
              <div className="text-[10px] text-white/30">Testing</div>
            </div>
            <div className="bg-[#12121a] rounded-lg p-2.5 text-center">
          ...
          {summary.total > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex-1 h-2 bg-[#12121a] rounded-full overflow-hidden flex">
          ...
          {Object.keys(summary.by_domain).length > 0 && (
            <div>
              <div className="text-[10px] text-white/25 uppercase tracking-wide mb-1.5">By Domain</div>
              <div className="space-y-1">
                {Object.entries(summary.by_domain).map(([domain, stats]) => (
                  <div key={domain} className="flex items-center gap-2 text-[10px]">
                    <span className="text-white/40 w-20 capitalize">{domain}</span>
                    <div className="flex-1 h-1.5 bg-[#12121a] rounded-full overflow-hidden flex">
                    ...
                    <span className="text-white/30 w-12 text-right">{stats.validated}/{stats.total}</span>
                  </div>
                ))}
              </div>
            </div>
