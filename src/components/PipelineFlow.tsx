        ...
        return (
          <div key={stage.key} className="flex items-center gap-3">
            <div className={`${colors.bg} ${colors.border} border rounded-xl px-6 py-4 min-w-[140px] text-center`}>
              ...
              <div className="text-sm font-medium text-[var(--text-primary)] mt-1">{stage.label}</div>
              <div className="text-xs text-[var(--text-muted)] mt-0.5">{stage.sublabel}</div>
              {stageCost && stageCost.costUsd > 0 && (
                <div className="text-[10px] text-[var(--text-faint)] mt-1">${stageCost.costUsd.toFixed(2)}</div>
              )}
            </div>
            {i < stages.length - 1 && (
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-[var(--text-faint)] flex-shrink-0">
