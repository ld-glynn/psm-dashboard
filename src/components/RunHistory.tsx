...
export function RunHistory({ serverAvailable, onRollback }: RunHistoryProps) {
  ...
  if (loading) return <div className="text-xs text-[var(--text-muted)]">Loading run history...</div>;
  if (runs.length === 0) return null;

  return (
    <div className="mt-3">
      <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-wide mb-2">Run History ({runs.length})</div>
      <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
        {[...runs].reverse().slice(0, 10).map((run) => {
          const cfg = statusConfig[run.status] || statusConfig.failed;
          const Icon = cfg.icon;
          return (
            <div key={run.run_id} className="flex items-center gap-2 bg-[var(--bg-input)] rounded-lg px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[var(--text-secondary)] font-mono">{run.run_id}</span>
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 text-[10px] rounded bg-[var(--text-faint)] text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--text-faint)] transition-colors disabled:opacity-30"
