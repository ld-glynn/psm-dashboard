...
export function TopBar({ onSearch }: { onSearch: () => void }) {
  return (
    <div className="h-12 border-b border-border bg-background/80 backdrop-blur-sm flex items-center px-4 gap-4">
      <button
        onClick={onSearch}
        className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-accent/20 border border-border text-muted-foreground hover:text-muted-foreground hover:border-ring transition-colors text-xs w-64"
      >
        <Search size={14} />
